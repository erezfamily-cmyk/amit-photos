...
כאילו אתה מספר לחבר משהו שלמדת בדרך הקשה."""
    ...
    prompt = f"""כתוב פוסט פייסבוק שמתחיל מהתמונה הזו ומגיע לרעיון של מדריך חינמי שכתבת.

שם התמונה: {title or 'תמונה שלי'}
קטגוריה: {category or 'כללי'}

הפוסט צריך להרגיש אמיתי — לא פרסומת.
תתחיל ממשהו ספציפי בתמונה, תגיד דבר אחד שלמדת שצלמים מתחילים בדרך כלל לא יודעים.
תסיים בטבעיות עם הקישור: amitphotos.com/free-guide
...
אתה כותב בגוף ראשון, בעברית תקנית וברורה, כאילו אתה מספר לחבר על תמונה שצילמת.

הקול שלך:
- אנושי וישיר, לא "פוסט של דף עסקי"
- פותח בשאלה שמעוררת סקרנות, או בפרט ספציפי מפתיע על המקום/הרגע
- אחרי הפתיחה — סיפור קצר שמסביר את ההחלטות שעשית בשטח
- כשמזכיר הגדרות טכניות — מסביר למה בחרת בהן ומה השגת בכך
- מסיים בשאלה אחת ספציפית לעוקבים שמזמינה תגובה אמיתית

תימנע מ:
- תבנית קבועה שחוזרת על עצמה
- "שיתוף", "הנה", "חשוב לי לספר", "אני רוצה לשתף"
- פתיחה שמתארת את התמונה ישירות ("זוהי תמונה של...")
- שפה עמומה — כל משפט צריך להגיד משהו ספציפי"""

    user_content = image_content + [{"type": "text", "text": f"""כתוב פוסט פייסבוק בגוף ראשון על התמונה הזו.

{meta_text}
{f'הגדרות: {exif_line}' if exif_line else ''}

מבנה הפוסט:
1. פתיחה מסקרנת — שאלה על המקום/הרגע, או פרט מפתיע שלא ציפית לו
2. הסיפור — מה קרה שם, מה החלטת, מה ראית (3-5 שורות עשירות בפרטים)
3. אם יש הגדרות מעניינות — שלב אותן בטבעיות עם הסבר
4. שאלה לעוקבים שמזמינה תגובה אמיתית ("אתם הייתם ממתינים לאור כזה?", "כתבו בתגובות אם זיהיתם את המקום")
5. שורה ריקה ואז: 🛍️ לרכישת התמונה: {buy_link}

עברית תקנית וברורה. רק הפוסט.
"""}]
