...
#### 📈 סטטיסטיקות (Analytics)

**KPI cards (שורה עליונה):**

| כרטיס | מה מציג |
|-------|---------|
| **צפיות השבוע** | סה"כ page views ב-7 הימים האחרונים + Δ% לעומת השבוע שעבר (ירוק = עלייה, אדום = ירידה) |
| **מדינה מובילה** | המדינה עם הכי הרבה מבקרים בחודש האחרון |
| **דף פופולרי** | סוג הדף הנצפה ביותר (בית, תמונה, מצלמה, ניתוח, מקום, משחקים, מבצע) |

**גרף 30 יום** — עמודות יומיות לפי page views.

**Top Pages** — גרף אופקי של כמות הצפיות לפי סוג דף. נתונים מצטברים מהרגע שהמעקב הופעל.

**פאנל המרה (תמונות):**

| עמודה | הסבר |
|-------|-------|
| 👁 צפיות | כמה פעמים נפתח הלייטבוקס לתמונה זו |
| 🛒 כוונה | כמה פעמים לחצו "כיצד לרכוש" |
| **% כוונה** | כוונה ÷ צפיות — מדד עניין ברכישה |
| 💰 רכישות | כמה רכישות בפועל |
| הכנסה | סה"כ הכנסה בשקלים |
| המרה | רכישות ÷ צפיות |

> **טיפ:** % כוונה גבוה (10%+) אבל המרה נמוכה — המחיר או ממשק הרכישה מרתיעים. % כוונה נמוך — התמונה פחות מושכת לרכישה.
...
## ניתוח תמונות (Photo Analysis)

### רשימת ניתוחים

בכל שורה: תמונה ממוזערת, כותרת, קטגוריה, סטטוס (פורסם/טיוטה).

| אינדיקטור | משמעות |
|-----------|---------|
| **EN✓** (ירוק) | קיים תוכן אנגלי לניתוח זה |
| 🌍 (כפתור) | לחץ לייצור/עדכון תרגום אנגלי אוטומטי ע"י Claude |

### עורך ניתוח (split-pane)

לחיצה על "ערוך" פותח עורך עם שני צדדים:
- **ימין** — שדות עריכה: כותרת, תיאור, קומפוזיציה (3 פסקאות), נתוני מצלמה, תגיות, annotations ויזואליות
- **שמאל** — תצוגה מיידית של הניתוח כפי שייראה לגולש

#### עריכת תוכן אנגלי

בתוך העורך, פתח את **"תוכן אנגלי (EN)"** (accordion):

| שדה | תוכן |
|-----|------|
| Composition Label 1-3 | כותרות הפסקאות באנגלית |
| Aperture / Shutter / ISO / Focal | הסברי מצלמה באנגלית |
| Tags (EN) | תגיות באנגלית (JSON array: `["tag1","tag2"]`) |
| Annotation Labels | תווית אנגלית לכל annotation על התמונה |

> **טיפ:** לחץ 🌍 בשורה הראשית לפני הכניסה לעריכה — Claude ייצר את כל התוכן האנגלי אוטומטית.
